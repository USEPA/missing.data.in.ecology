...
out <- parLapply(cl, 1:nsim, safely(get_trial), rho_y_ry = 0.9, mu_probit_ry = -0.5)
stopCluster(cl)
...
write_csv(out_success, here("inst", "output", "simulation", "mnar_all_large.csv"))
write_csv(out_summary, here("inst", "output", "simulation", "mnar_summary_large.csv"))
