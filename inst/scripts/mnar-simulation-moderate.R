...
out <- parLapply(cl, 1:nsim, safely(get_trial), rho_y_ry = 0.5, mu_probit_ry = -0.5)
stopCluster(cl)
...
write_csv(out_success, here("inst", "output", "simulation", "mnar_all_moderate.csv"))
write_csv(out_summary, here("inst", "output", "simulation", "mnar_summary_moderate.csv"))
